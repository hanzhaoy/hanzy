...
public class DateToDate {
    /***************************************************************************
     * 在beginDate和endDate之间所包含假期的天数，若不包含则返回0；
     * 假期和周末分别进行单独考虑！
     * @param list
     * @param beginDate
     * @param endDate
    ...
    public static int getContainVacation(List list, Date beginDate, Date endDate) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if ((tempDate.after(endDate)) || (tempDate.before(beginDate))) {
                ...
                while (tempDay < endcal.get(Calendar.DAY_OF_YEAR)
                    ...
                    //节假日和周末可能有重叠的情况！
                    if ((tempcal.get(Calendar.DAY_OF_WEEK)) == 2 || (tempcal.get(Calendar.DAY_OF_WEEK) == 1)) {
                        System.out.println((tempcal.get(Calendar.DAY_OF_WEEK)) == 2);
                        System.out.println((tempcal.get(Calendar.DAY_OF_WEEK) == 1));
                        System.out.println("节假日和周末重叠的情况！" + tempcal.get(Calendar.DAY_OF_WEEK));
                        days--;
                    }
        ...
        for (int j = begincal.get(Calendar.DAY_OF_YEAR); j <= endcal.get(Calendar.DAY_OF_YEAR); j++) {
